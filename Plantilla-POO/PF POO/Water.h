...
public:
	...
	void Draw()
	{
		...
		glEnable(GL_BLEND);
		static float water_factor_min = 35.0f;
		static float water_factor_max = 25.0f;
		static float water_size = 150;
		water_factor_min += .002f;
		water_factor_max += .002f;
		if (water_factor_min > 45)
		{
			water_factor_min = 25.0f;
			water_factor_max = 35.0f;
		}
		glBindTexture(GL_TEXTURE_2D, planoTextura);
		glBegin(GL_QUADS);
